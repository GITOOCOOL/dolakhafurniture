...
import { Box, Search, Check, AlertTriangle, EyeOff, Facebook, Filter, X, ChevronDown, Tag, Plus, RefreshCcw, Edit3, CheckCircle2 } from "lucide-react";
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
        <div>
           <h2 className="text-3xl font-serif italic text-heading leading-tight">Collection Ledger</h2>
           <p className="text-[10px] font-sans font-bold uppercase tracking-[0.2em] text-label opacity-60">Inventory & Piece Management</p>
        </div>
        <Button 
          onClick={() => {
               ...
               <div className="flex flex-wrap items-center gap-4">
                  {Object.entries({
                    ...
                    >
                       ...
                       />
                       <div className={`w-4 h-4 rounded-md border flex items-center justify-center transition-all ${visibleColumns[key as keyof typeof visibleColumns] ? 'bg-action border-action text-white' : 'border-soft'}`}>
                          {visibleColumns[key as keyof typeof visibleColumns] && <Check size={10} strokeWidth={4} />}
                       </div>
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
              <tr className="border-b border-soft border-dotted bg-soft/10">
                ...
                {visibleColumns.price && <th className="px-8 py-6 text-[9px] font-sans font-bold uppercase tracking-widest text-label w-40">Price (Rs.)</th>}
                {visibleColumns.specs && <th className="px-8 py-6 text-[9px] font-sans font-bold uppercase tracking-widest text-label w-40">Specs (L×B×H)</th>}
                {visibleColumns.stock && <th className="px-8 py-6 text-[9px] font-sans font-bold uppercase tracking-widest text-label w-32">Stock</th>}
                {visibleColumns.status && <th className="px-8 py-6 text-[9px] font-sans font-bold uppercase tracking-widest text-label text-center w-32">Status</th>}
                <th className="px-8 py-6 text-[9px] font-sans font-bold uppercase tracking-widest text-label text-center w-24">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-soft divide-dotted">
              {filteredProducts.map((product) => (
                <tr key={product._id} className="hover:bg-soft/5 transition-colors group">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-1 text-[10px] font-bold text-heading">
                           <span className="opacity-40 font-normal">L:</span> {product.length || "—"}
                           <span className="opacity-40 font-normal ml-1">B:</span> {product.breadth || "—"}
                           <span className="opacity-40 font-normal ml-1">H:</span> {product.height || "—"}
                        </div>
                        <p className="text-[8px] uppercase tracking-tighter text-label opacity-40">inches</p>
                      </div>
                    </td>
                  )}
                    ...
                    <td className="px-8 py-6 text-center">
                      <div className="flex items-center justify-center gap-3">
                         <div 
                           ...
                           title={product.isActive ? "Visible on Web" : "Hidden from Web"}
                         >
                           {product.isActive ? <CheckCircle2 size={14} /> : <EyeOff size={14} />}
                         </div>
